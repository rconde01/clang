...
bool
switchesFormatting(const FormatToken & Token)
{
                                              ...
                                              Split    Split) const
{
   ...
   // single space, like: lala lala
   return RemainingTokenColumns + 1 - Split.second;
}

unsigned
BreakableSingleLineToken::getLineCount() const
{
   return 1;
}

unsigned
BreakableSingleLineToken::getLineLengthAfterSplit(
    unsigned LineIndex, unsigned TailOffset, StringRef::size_type Length) const
{
...
: BreakableSingleLineToken(
      Tok, StartColumn, Prefix, Postfix, InPPDirective, Encoding, Style)
{
}

BreakableToken::Split
BreakableStringLiteral::getSplit(unsigned      LineIndex,
                                 unsigned      TailOffset,
                                 unsigned      ColumnLimit,
...
void
BreakableStringLiteral::insertBreak(unsigned            LineIndex,
                                    unsigned            TailOffset,
                                    Split               Split,
                                    WhitespaceManager & Whitespaces)
{
   Whitespaces.replaceWhitespaceInToken(Tok,
                                        Prefix.size() + TailOffset
                                            + Split.first,
                                        Split.second,
                                        Postfix,
                                        Prefix,
...
unsigned
BreakableComment::getLineCount() const
{
   return Lines.size();
}

BreakableToken::Split
BreakableComment::getSplit(unsigned      LineIndex,
                           unsigned      TailOffset,
                           unsigned      ColumnLimit,
                           llvm::Regex & CommentPragmasRegex) const
{
...
void
BreakableComment::compressWhitespace(unsigned            LineIndex,
                                     unsigned            TailOffset,
                                     Split               Split,
                                     WhitespaceManager & Whitespaces)
{
   ...
   Whitespaces.replaceWhitespaceInToken(tokenAt(LineIndex),
                                        BreakOffsetInToken,
                                        CharsToRemove,
                                        "",
                                        "",
...
BreakableToken::Split
BreakableComment::getReflowSplit(StringRef Text,
                                 ...
                                 unsigned  ColumnLimit) const
{
   ...
   unsigned FullWidth =
       ...
       + encoding::columnWidthWithTabs(
             TrimmedText, ReflowStartColumn, Style.TabWidth, Encoding);
   ...
   // line if it can't be broken into smaller pieces (like when the first word
   // of a long line is longer than the column limit), it's not OK to reflow
   // that long word up. So we recompute the size of the previous line after
   // reflowing and only return the reflow split if that's under the line limit.
...
const FormatToken &
BreakableComment::tokenAt(unsigned LineIndex) const
{
   return Tokens[LineIndex] ? *Tokens[LineIndex] : Tok;
}

static bool
mayReflowContent(StringRef Content)
{
...
BreakableBlockComment::BreakableBlockComment(const FormatToken & Token,
                                             unsigned            StartColumn,
...
: BreakableComment(Token, StartColumn, InPPDirective, Encoding, Style)
{
   ...
   for(size_t i = 1, e = Lines.size(); i < e; ++i)
   {
      if(Content[i].empty())
      {
         if(i + 1 == e)
         {
            // Empty last line means that we already have a star as a part of
            // the trailing */. We also need to preserve whitespace, so that */
         ...
         {
            // For all other lines, set the start column to 0 if they're empty,
            // so we do not insert trailing whitespace anywhere.
            ContentColumn[i] = 0;
         }
         continue;
      }
...
void
BreakableBlockComment::adjustWhitespace(unsigned LineIndex, int IndentDelta)
{
   // When in a preprocessor directive, the trailing backslash in a block
   // comment is not needed, but can serve a purpose of uniformity with
   // necessary escaped newlines outside the comment. In this case we remove it
   // here before trimming the trailing whitespace. The backslash will be
   // re-added later when inserting a line break.
...
unsigned
BreakableBlockComment::getLineLengthAfterSplit(
    unsigned LineIndex, unsigned TailOffset, StringRef::size_type Length) const
{
   unsigned ContentStartColumn = getContentStartColumn(LineIndex, TailOffset);
   unsigned LineLength         = ContentStartColumn
                         + encoding::columnWidthWithTabs(
   ...
   {
      LineLength += 2;
      // We never need a decoration when breaking just the trailing "*/"
      // postfix. Note that checking that Length == 0 is not enough, since
      // Length could also be StringRef::npos.
...
void
BreakableBlockComment::insertBreak(unsigned            LineIndex,
                                   unsigned            TailOffset,
                                   Split               Split,
                                   WhitespaceManager & Whitespaces)
{
   ...
   // all the lines except for the last one, where the star in "*/" acts as a
   // decoration.
   ...
   // offset from the beginning of the token text.
   unsigned BreakOffsetInToken =
       Text.data() - tokenAt(LineIndex).TokenText.data() + Split.first;
   unsigned CharsToRemove = Split.second;
...
BreakableToken::Split
BreakableBlockComment::getSplitBefore(unsigned      LineIndex,
                                      unsigned      PreviousEndColumn,
                                      unsigned      ColumnLimit,
                                      llvm::Regex & CommentPragmasRegex) const
{
   ...
   return getReflowSplit(
       TrimmedContent, ReflowPrefix, PreviousEndColumn, ColumnLimit);
}

unsigned
BreakableBlockComment::getReflownColumn(StringRef Content,
                                        unsigned  LineIndex,
...
unsigned
BreakableBlockComment::getLineLengthAfterSplitBefore(unsigned LineIndex,
                                                     unsigned TailOffset,
                                                     unsigned PreviousEndColumn,
                                                     unsigned ColumnLimit,
                                               ...
                                               WhitespaceManager & Whitespaces)
{
   ...
   if(SplitBefore.first != StringRef::npos)
   {
      ...
      // the content. Note that we don't need a penalty for this break, since it
      // doesn't change the total number of lines.
   ...
   if(Content[LineIndex].empty())
   {
      if(LineIndex + 1 == Lines.size())
      {
         ...
         {
            // If the last line was empty, we don't need a prefix, as the */
            // will line up with the decoration (if it exists).
            Prefix = "";
         }
      }
      else if(!Decoration.empty())
...
bool
BreakableBlockComment::mayReflow(unsigned      LineIndex,
                                 llvm::Regex & CommentPragmasRegex) const
{
   // Content[LineIndex] may exclude the indent after the '*' decoration. In
   // that case, we compute the start of the comment pragma manually.
       ...
       CurrentTok = CurrentTok->Next)
   {
      ...
      for(size_t i = FirstLineIndex, e = Lines.size(); i < e; ++i)
      {
         ...
         OriginalContentColumn[i] =
             StartColumn
             + encoding::columnWidthWithTabs(
                   OriginalPrefix[i], StartColumn, Style.TabWidth, Encoding);
         ContentColumn[i] =
             StartColumn
             + encoding::columnWidthWithTabs(
                   Prefix[i], StartColumn, Style.TabWidth, Encoding);
      ...
      if(CurrentTok->Next && !CurrentTok->Next->ContinuesLineCommentSection)
      {
         ...
         // would split the contents of the enum into two unwrapped lines in
         // this example, which is undesirable: enum A {
         //   a, // comment about a
         //
         //   // comment about b
         //   b
...
unsigned
BreakableLineCommentSection::getLineLengthAfterSplit(
    unsigned LineIndex, unsigned TailOffset, StringRef::size_type Length) const
{
   unsigned ContentStartColumn =
...
void
BreakableLineCommentSection::insertBreak(unsigned            LineIndex,
                                         unsigned            TailOffset,
                                         Split               Split,
                                         WhitespaceManager & Whitespaces)
{
   ...
   unsigned BreakOffsetInToken =
       Text.data() - tokenAt(LineIndex).TokenText.data() + Split.first;
   unsigned CharsToRemove = Split.second;
   // Compute the size of the new indent, including the size of the new prefix
   // of the newly broken line.
   ...
   Whitespaces.replaceWhitespaceInToken(tokenAt(LineIndex),
                                        ...
                                        InPPDirective,
                                        /*Newlines=*/1,
                                        /*Spaces=*/IndentAtLineBreak
                                            - Prefix[LineIndex].size());
}

BreakableComment::Split
BreakableLineCommentSection::getSplitBefore(
    unsigned      LineIndex,
    unsigned      PreviousEndColumn,
    unsigned      ColumnLimit,
    llvm::Regex & CommentPragmasRegex) const
{
   if(!mayReflow(LineIndex, CommentPragmasRegex))
      return Split(StringRef::npos, 0);
   return getReflowSplit(
       Content[LineIndex], ReflowPrefix, PreviousEndColumn, ColumnLimit);
}

unsigned
BreakableLineCommentSection::getLineLengthAfterSplitBefore(
    unsigned LineIndex,
    unsigned TailOffset,
    unsigned PreviousEndColumn,
    unsigned ColumnLimit,
    Split    SplitBefore) const
{
   ...
   else
   {
      ...
      return StartColumn
             + encoding::columnWidthWithTabs(
                   Content[LineIndex], StartColumn, Style.TabWidth, Encoding);
   }
}

void
BreakableLineCommentSection::replaceWhitespaceBefore(
    unsigned            LineIndex,
    unsigned            PreviousEndColumn,
    unsigned            ColumnLimit,
    Split               SplitBefore,
  ...
  // // line 2
   if(LineIndex > 0 && Tokens[LineIndex] != Tokens[LineIndex - 1])
   {
      ...
      else
      {
         ...
         // previous token is necessary. However, we still may need to adjust
         // the start column. Note that ContentColumn[LineIndex] is the expected
         // content column after a possible update to the prefix, hence the
         // prefix length change is included.
         unsigned LineColumn =
             ContentColumn[LineIndex]
         ...
         // untouchable token, even if LineColumn is the same as the original
         // column of the token. This is because WhitespaceManager doesn't align
         // trailing comments if they are untouchable.
         Whitespaces.replaceWhitespace(*Tokens[LineIndex],
                                       /*Newlines=*/1,
                                       /*Spaces=*/LineColumn,
...
void
BreakableLineCommentSection::updateNextToken(LineState & State) const
{
   if(LastLineTok)
   {
...
bool
BreakableLineCommentSection::mayReflow(unsigned      LineIndex,
                                       llvm::Regex & CommentPragmasRegex) const
{
