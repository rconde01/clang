...
public:
   ...
   /// line".
   virtual unsigned
   getLineLengthAfterSplit(unsigned             LineIndex,
                           unsigned             TailOffset,
   ...
   /// range \p Split has been compressed into a single space.
   ...
   /// space.
   virtual void compressWhitespace(unsigned            LineIndex,
                                   unsigned            TailOffset,
                                   Split               Split,
                                   WhitespaceManager & Whitespaces) = 0;
   ...
   /// \brief Returns the number of columns required to format the piece of line
   /// at \p LineIndex after the content preceding the whitespace range
   /// specified \p SplitBefore has been reformatted, but before any breaks are
   /// made to this line.
...
protected:
   BreakableToken(const FormatToken & Tok,
                  bool                InPPDirective,
                  encoding::Encoding  Encoding,
                  const FormatStyle & Style)
   : Tok(Tok), InPPDirective(InPPDirective), Encoding(Encoding), Style(Style)
   {
   }
...
protected:
   BreakableSingleLineToken(const FormatToken & Tok,
...
public:
   ...
   /// after formatting.
   BreakableStringLiteral(const FormatToken & Tok,
                          unsigned            StartColumn,
                          StringRef           Prefix,
                          StringRef           Postfix,
                          bool                InPPDirective,
   ...
   Split getSplit(unsigned      LineIndex,
                  unsigned      TailOffset,
                  unsigned      ColumnLimit,
                  llvm::Regex & CommentPragmasRegex) const override;
   void  insertBreak(unsigned            LineIndex,
                     unsigned            TailOffset,
                     Split               Split,
                     WhitespaceManager & Whitespaces) override;
   void  compressWhitespace(unsigned            LineIndex,
                            unsigned            TailOffset,
                            Split               Split,
                            WhitespaceManager & Whitespaces) override
   {
...
protected:
   ...
   /// formatting.
   BreakableComment(const FormatToken & Token,
                    unsigned            StartColumn,
                    bool                InPPDirective,
                    encoding::Encoding  Encoding,
                    const FormatStyle & Style);
...
protected:
   ...
   Split getReflowSplit(StringRef Text,
   ...
   // whitespace between the lines. Note that the decoration (if present) is
   // also not considered part of the text.
...
{
public:
   BreakableBlockComment(const FormatToken & Token,
                         unsigned            StartColumn,
   ...
   void     insertBreak(unsigned            LineIndex,
                        ...
                        Split               Split,
                        WhitespaceManager & Whitespaces) override;
   Split    getSplitBefore(unsigned      LineIndex,
                           unsigned      PreviousEndColumn,
                           unsigned      ColumnLimit,
                           llvm::Regex & CommentPragmasRegex) const override;
...
{
public:
   BreakableLineCommentSection(const FormatToken & Token,
                               unsigned            StartColumn,
                               unsigned            OriginalStartColumn,
                               bool                FirstInLine,
                               bool                InPPDirective,
...
private:
   ...
   ///
   /// The distinction is because if the token of the last line of this
   /// breakable token is distinct from the initial token, this breakable token
   /// owns the whitespace before the token of the last line, and the whitespace
   /// manager must be able to modify it.
