...
static unsigned
getLengthToMatchingParen(const FormatToken & Tok)
{
   if(!Tok.MatchingPairedToken)
      return 0;
   FormatToken * End = Tok.MatchingPairedToken;
...
static unsigned
getLengthToNextOperator(const FormatToken & Tok)
{
   if(!Tok.NextOperator)
      return 0;
...
static bool
startsSegmentOfBuilderTypeCall(const FormatToken & Tok)
{
...
static bool
startsNextParameter(const FormatToken & Current, const FormatStyle & Style)
{
                                      ...
                                      bool                  DryRun)
{
   ...
   State.Stack.push_back(ParenState(FirstIndent,
                                    FirstIndent,
...
bool
ContinuationIndenter::canBreak(const LineState & State)
{
...
bool
ContinuationIndenter::mustBreak(const LineState & State)
{
   const FormatToken & Current  = *State.NextToken;
   const FormatToken & Previous = *Current.Previous;
   ...
   if((startsNextParameter(Current, Style) || Previous.is(tok::semi)
       || (Previous.is(TT_TemplateCloser) && Current.is(TT_StartOfName) &&
           ...
           // clang-format sets BreakBeforeParameter to avoid bin packing and
           // this creates a completely unnecessary line break after a template
           // type that isn't line-wrapped.
      ...
      return true;
   if(((Previous.is(TT_ArrayInitializerLSquare) && Previous.ParameterCount > 1))
      && Style.ColumnLimit > 0
...
unsigned
ContinuationIndenter::addTokenToState(LineState & State,
                                      bool        Newline,
                                      bool        DryRun,
                                      unsigned    ExtraSpaces)
{
   ...
   {
      unsigned EndColumn =
          SourceMgr.getSpellingColumnNumber(Current.PrecedingWhitespaceRange.getEnd());
      if(Current.LastNewlineOffset != 0)
      {
         // If there is a newline within this token, the final column will
         // solely determined by the current end column.
         State.Column = EndColumn;
      }
      else
      {
         unsigned StartColumn = SourceMgr.getSpellingColumnNumber(
             Current.PrecedingWhitespaceRange.getBegin());
...
void
ContinuationIndenter::addTokenOnCurrentLine(LineState & State,
                                            bool        DryRun,
                                            unsigned    ExtraSpaces)
{
      ...
      && State.Stack.back().VariablePos == 0)
   {
      ...
      while(Tok && State.Stack.back().VariablePos >= Tok->FirstLineColumnWidth)
      {
         State.Stack.back().VariablePos -= Tok->FirstLineColumnWidth;
   ...
   if(!DryRun)
      Whitespaces.replaceWhitespace(
          Current, /*Newlines=*/0, Spaces, State.Column + Spaces);
   ...
   if(Current.isMemberAccess() && Previous.is(tok::r_paren)
      && (Previous.MatchingPairedToken
          && (Previous.TotalLength - Previous.MatchingPairedToken->TotalLength > 10)))
   ...
   else if((Previous.isOneOf(
               TT_BinaryOperator, TT_ConditionalExpr, TT_CtorInitializerColon))
           && ((Previous.getPrecedence() != prec::Assignment
   ...
   else if(Previous.opensScope())
   {
      ...
      if(Previous.MatchingPairedToken)
      {
         const FormatToken * Next = Previous.MatchingPairedToken->getNextNonComment();
...
unsigned
ContinuationIndenter::addTokenOnNewLine(LineState & State, bool DryRun)
{
   FormatToken &       Current  = *State.NextToken;
   const FormatToken & Previous = *State.NextToken->Previous;
   ...
   if(NextNonComment->is(tok::lessless) && State.Stack.back().FirstLessLess == 0
   ...
   if((PreviousNonComment && PreviousNonComment->isOneOf(tok::comma, tok::semi)
       && !State.Stack.back().AvoidBinPacking)
      || Previous.is(TT_BinaryOperator))
      State.Stack.back().BreakBeforeParameter = false;
   if(Previous.is(TT_TemplateCloser) && Current.NestingLevel == 0)
      State.Stack.back().BreakBeforeParameter = false;
   ...
   if(!DryRun)
   {
      unsigned Newlines = std::max(
          1u, std::min(Current.UserNewlinesBefore, Style.MaxEmptyLinesToKeep + 1));
      bool ContinuePPDirective =
          State.Line->InPPDirective && State.Line->Type != LT_ImportStatement;
      Whitespaces.replaceWhitespace(
          Current, Newlines, State.Column, State.Column, ContinuePPDirective);
   }
...
ContinuationIndenter::getNewLineColumn(const LineState & State)
{
   ...
   if(Current.isOneOf(tok::r_brace, tok::r_square) && State.Stack.size() > 1)
   {
      ...
      if(Current.MatchingPairedToken
         && Current.MatchingPairedToken->BlockKind == BK_BracedInit)
         return State.Stack[State.Stack.size() - 2].LastSpace;
      return State.FirstIndent;
   }
   // Indent a closing parenthesis at the previous level if followed by a semi
   // or opening brace. This allows indentations such as:
   //     foo(
   //       a,
   //     );
   //     function foo(
   ...
   if(PreviousNonComment && PreviousNonComment->is(TT_CtorInitializerColon)
      && Style.BreakConstructorInitializers == FormatStyle::BCIS_AfterColon)
      return State.Stack.back().Indent;
   if(NextNonComment->isOneOf(
          TT_CtorInitializerColon, TT_InheritanceColon, TT_InheritanceComma))
                                           ...
                                           bool        Newline)
{
   ...
   if(Current.isOneOf(TT_BinaryOperator, TT_ConditionalExpr) && Newline)
      State.Stack.back().NestedBlockIndent =
          State.Column + Current.FirstLineColumnWidth + 1;
   ...
   State.Column += Current.FirstLineColumnWidth;
   ...
   // role (comma separated list). That way, the decision whether or not to
   // break after the "{" is already done and both options are tried and
   // evaluated.
...
void
ContinuationIndenter::moveStatePastFakeLParens(LineState & State, bool Newline)
{
   const FormatToken & Current  = *State.NextToken;
   const FormatToken * Previous = Current.getPreviousNonComment();
...
void
ContinuationIndenter::moveStatePastScopeOpener(LineState & State, bool Newline)
{
   const FormatToken & Current = *State.NextToken;
   if(!Current.opensScope())
      return;

   if(Current.MatchingPairedToken && Current.BlockKind == BK_Block)
   {
   ...
   if(Current.isOneOf(tok::l_brace, TT_ArrayInitializerLSquare))
   {
      ...
      {
         NewIndent =
             State.Stack.back().LastSpace + Style.ContinuationIndentWidth;
      }
      const FormatToken * NextNoComment = Current.getNextNonComment();
      bool                EndsInComma   = Current.MatchingPairedToken
                         && Current.MatchingPairedToken->Previous
                         && Current.MatchingPairedToken->Previous->is(tok::comma);
      AvoidBinPacking =
          EndsInComma || !Style.BinPackArguments
          || (NextNoComment
   ...
   else
   {
      ...
      // Ensure that different different brackets force relative alignment,
      // e.g.: void SomeFunction(vector<  // break
      //                       int> v);
   ...
   bool NoLineBreak = Current.Children.empty()
...
unsigned
ContinuationIndenter::addMultilineToken(const FormatToken & Current,
                                        LineState &         State)
{
                                           ...
                                           bool                DryRun)
{
   ...
   unsigned StartColumn = State.Column - Current.FirstLineColumnWidth;
   ...
   if(Current.isStringLiteral())
   {
      ...
      // reduce the overhead) for each FormatToken, which is a string, so that
      // we don't run multiple checks here on the hot path.
      if((Text.endswith(Postfix = "\"")
         ...
         || (Text.startswith(Prefix = "_T(\"")
             && Text.endswith(Postfix = "\")")))
      {
         Token.reset(new BreakableStringLiteral(Current,
                                                StartColumn,
                                                Prefix,
                                                Postfix,
   ...
   else if(Current.is(TT_BlockComment))
   {
      if(!Current.isTrailingComment() || !Style.ReflowComments ||
         ...
         return addMultilineToken(Current, State);
      Token.reset(new BreakableBlockComment(Current,
                                            StartColumn,
               ...
               || Current.Previous->isNot(TT_ImplicitStringLiteral)))
   {
      if(!Style.ReflowComments
         ...
         || switchesFormatting(Current))
         return 0;
      Token.reset(new BreakableLineCommentSection(Current,
                                                  StartColumn,
                                                  Current.OriginalColumn,
                                                  !Current.Previous,
                                                  /*InPPDirective=*/false,
   ...
   // a line that doesn't get reflown with the previous line is reached.
       ...
       ++LineIndex)
   {
      ...
      if(ReflowInProgress)
      {
         SplitBefore = Token->getSplitBefore(LineIndex,
      ...
      if(!DryRun)
         Token->replaceWhitespaceBefore(LineIndex,
                                        RemainingTokenColumns,
                                        RemainingSpace,
                                        SplitBefore,
                                        Whitespaces);
      RemainingTokenColumns =
          Token->getLineLengthAfterSplitBefore(LineIndex,
                                               TailOffset,
                                               RemainingTokenColumns,
                                               ColumnLimit,
                                               SplitBefore);
      while(RemainingTokenColumns > RemainingSpace)
      {
         ...
         // compression instead of inserting a line break.
         ...
         if(RemainingTokenColumnsAfterCompression <= RemainingSpace)
         {
            ...
            if(!DryRun)
               Token->compressWhitespace(
                   LineIndex, TailOffset, Split, Whitespaces);
            break;
         }
         ...
         // columns, but will still be expanded to the next tab stop, so we
         // don't save any columns.
...
unsigned
ContinuationIndenter::getColumnLimit(const LineState & State) const
{
...
bool
ContinuationIndenter::nextIsMultilineString(const LineState & State)
{
   const FormatToken & Current = *State.NextToken;
   ...
   if(Style.ColumnLimit != 0
      && State.Column + Current.FirstLineColumnWidth + Current.UnbreakableTailLength
             > Style.ColumnLimit)
