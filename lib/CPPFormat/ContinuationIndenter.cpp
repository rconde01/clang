...
static bool
opensProtoMessageField(const FormatToken & LessTok, const FormatStyle & Style)
{
                                            ...
                                            unsigned    ExtraSpaces)
{
   ...
   if(Current.is(TT_SelectorName) && !State.Stack.back().ObjCSelectorNameFound)
   {
      ...
      if(Current.LongestObjCSelectorName == 0)
         ...
         State.Stack.back().ColonPos =
             MinIndent + Current.LongestObjCSelectorName;
      else
          ...
          || Previous.Previous->endsSequence(tok::kw_constexpr, tok::kw_if)))
   {
      ...
      State.Stack.back().NestedBlockIndent = State.Column;
           ...
           && ((Previous.is(tok::comma) && !Previous.is(TT_OverloadedOperator))
...
ContinuationIndenter::addTokenOnNewLine(LineState & State, bool DryRun)
{
   ...
   else if(NextNonComment->is(TT_SelectorName))
   {
      ...
      {
         State.Stack.back().ColonPos =
             State.Column + NextNonComment->ColumnWidth;
      }
   }
           ...
           && PreviousNonComment->isOneOf(TT_ObjCMethodExpr, TT_DictLiteral))
   {
      ...
      //                            // ...
      //                        }];
      // Thus, we set LastSpace of the next higher NestingLevel, to which we
      // move when we consume all of the "}"'s FakeRParens at the "{".
...
ContinuationIndenter::moveStatePastScopeOpener(LineState & State, bool Newline)
{
   ...
   else
   {
      ...
      // nested expressions relative to the ${}. Otherwise, this can create
      // quite a mess.
      ...
      if(Current.is(TT_ObjCMethodExpr) && Current.MatchingParen)
      {
         ...
         {
            // For ColumnLimit = 0, we have to figure out whether there is or
            // has to be a line break within this call.
                                           ...
                                           bool                DryRun)
{
   ...
   {
      // FIXME: String literal breaking is currently disabled for Java and JS,
      // as it requires strings to be merged using "+" which we don't support.
      if(Style.Language == FormatStyle::LK_Java
         || Style.Language == FormatStyle::LK_JavaScript
