...
public:
   ...
   LineState getInitialState(unsigned              FirstIndent,
   ...
   /// \c Replacement.
   unsigned addTokenToState(LineState & State,
                            bool        Newline,
                            bool        DryRun,
...
private:
   ...
   /// \brief Update 'State' according to the next token's fake left
   /// parentheses.
   ...
   unsigned breakProtrudingToken(const FormatToken & Current,
                                 LineState &         State,
                                 bool                DryRun);
   ...
   /// \c Replacement.
   void
   addTokenOnCurrentLine(LineState & State, bool DryRun, unsigned ExtraSpaces);
   ...
   /// handled in \c addNextStateToQueue, and the penalty for other lines
   /// doesn't matter, as we don't change them.
   ...
   /// literal.
   ///
   /// This includes implicitly concatenated strings, strings that will be
   /// broken by clang-format and string literals with escaped newlines.
