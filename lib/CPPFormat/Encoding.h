...
/// \p Encoding.
inline unsigned
columnWidth(StringRef Text, Encoding Encoding)
{
   if(Encoding == Encoding_UTF8)
   {
      int ContentWidth = llvm::sys::unicode::columnWidthUTF8(Text);
      // FIXME: Figure out the correct way to handle this in the presence of
      // both printable and unprintable multi-byte UTF-8 characters. Falling
      // back to returning the number of bytes may cause problems, as
      // columnWidth suddenly becomes non-additive.
      if(ContentWidth >= 0)
         return ContentWidth;
   }
   return Text.size();
...
inline unsigned
columnWidthWithTabs(StringRef Text,
...
inline unsigned
getCodePointNumBytes(char FirstChar, Encoding Encoding)
{
   switch(Encoding)
   {
   case Encoding_UTF8:
...
inline unsigned
getEscapeSequenceLength(StringRef Text)
{
   assert(Text[0] == '\\');
   if(Text.size() < 2)
      return 1;
