...
struct ScalarEnumerationTraits<FormatStyle::ReturnTypeBreakingStyle>
{
   static void enumeration(IO &                                   IO,
                           FormatStyle::ReturnTypeBreakingStyle & Value)
   {
      IO.enumCase(Value, "None", FormatStyle::RTBS_None);
      IO.enumCase(Value, "All", FormatStyle::RTBS_All);
      IO.enumCase(Value, "TopLevel", FormatStyle::RTBS_TopLevel);
      IO.enumCase(
          Value, "TopLevelDefinitions", FormatStyle::RTBS_TopLevelDefinitions);
...
struct ScalarEnumerationTraits<FormatStyle::SpaceBeforeParensOptions>
{
                           ...
                           FormatStyle::SpaceBeforeParensOptions & Value)
   {
      ...
      IO.enumCase(
          Value, "ControlStatements", FormatStyle::SBPO_ControlStatements);
   ...
   static void mapping(IO & IO, FormatStyle & Style)
   {
      if(IO.outputting())
      {
         StringRef StylesArray[] = {
             "LLVM", "Google", "Chromium", "Mozilla", "WebKit", "GNU"};
...
template<>
struct MappingTraits<FormatStyle::IncludeCategory>
{
...
struct DocumentListTraits<std::vector<FormatStyle>>
{
   ...
   static FormatStyle &
   element(IO & IO, std::vector<FormatStyle> & Seq, size_t Index)
   {
      if(Index >= Seq.size())
      {
...
inline llvm::Error
make_string_error(const llvm::Twine & Message)
{
...
const char *
ParseErrorCategory::name() const noexcept
{
   return "clang-format.parse_error";
}

std::string
ParseErrorCategory::message(int EV) const
{
...
static FormatStyle
expandPresets(const FormatStyle & Style)
{
   if(Style.BreakBeforeBraces == FormatStyle::BS_Custom)
      return Style;
   FormatStyle Expanded   = Style;
   Expanded.BraceWrapping = {false,
                             false,
                             false,
                             false,
                             false,
...
std::string
configurationAsText(const FormatStyle & Style)
{
   std::string              Text;
...
{
public:
   Formatter(const Environment &       Env,
           ...
           FormatTokenLexer &                 Tokens) override
   {
      ...
      ContinuationIndenter Indenter(Style,
                                    Tokens.getKeywords(),
                                    Env.getSourceManager(),
                                    Whitespaces,
                                    Encoding,
                                    BinPackInconclusiveFunctions);
      UnwrappedLineFormatter(&Indenter,
                             &Whitespaces,
                             Style,
                             Tokens.getKeywords(),
                             Env.getSourceManager(),
...
private:
   ...
   // The function checks if the namespace, which starts from \p CurrentLine,
   // and its nested namespaces are empty and delete them if they are empty. It
   // also sets \p NewLine to the last line checked. Returns true if the current
   // namespace is empty.
                            ...
                            std::set<unsigned> &               DeletedLines)
   {
      ...
      {
         // If the left brace is in a new line, we should consume it first so
         // that it does not make the namespace non-empty.
      ...
      while(++CurrentLine < End)
      {
                                                       ...
                                                       tok::kw_namespace))
         {
            if(!checkEmptyNamespace(
                   AnnotatedLines, CurrentLine, NewLine, DeletedLines))
               return false;
            CurrentLine = NewLine;
            continue;
         }
   ...
   // one of the token in the pair if the left token has \p LK token kind and
   // the right token has \p RK token kind. If \p DeleteLeft is true, the left
   // token is deleted on match; otherwise, the right token is deleted.
   template<typename LeftKind, typename RightKind>
   void
   cleanupPair(FormatToken * Start, LeftKind LK, RightKind RK, bool DeleteLeft)
   {
   ...
   tooling::Replacements generateFixes()
   {
      ...
      // affected ranges more efficient when we run reformat on the changed
      // code.
      unsigned Idx = 0;
...
static bool
affectsRange(ArrayRef<tooling::Range> Ranges, unsigned Start, unsigned End)
{
...
static void
sortCppIncludes(const FormatStyle &                       Style,
                ...
                unsigned *               Cursor)
{
   ...
   {
      auto      Pos  = Code.find('\n', SearchFrom);
      StringRef Line = Code.substr(
          Prev, (Pos != StringRef::npos ? Pos : Code.size()) - Prev);
      ...
      if(!FormattingOff && !Line.endswith("\\"))
      {
         ...
         {
            sortCppIncludes(
                Style, IncludesInBlock, Ranges, FileName, Replaces, Cursor);
...
formatReplacements(StringRef                     Code,
                   const tooling::Replacements & Replaces,
                   const FormatStyle &           Style)
{
   // We need to use lambda function here since there are two versions of
   // `sortIncludes`.
   auto SortIncludes = [](const FormatStyle &         Style,
                          StringRef                   Code,
   ...
   // We need to use lambda function here since there are two versions of
   // `reformat`.
   auto Reformat = [](const FormatStyle &         Style,
                      StringRef                   Code,
                      std::vector<tooling::Range> Ranges,
                      StringRef FileName) -> tooling::Replacements {
...
inline bool
isHeaderInsertion(const tooling::Replacement & Replace)
{
...
inline bool
isHeaderDeletion(const tooling::Replacement & Replace)
{
        ...
        GetOffsetAfterSequence)
{
   ...
   Lexer                 Lex(Env->getFileID(),
             SourceMgr.getBuffer(Env->getFileID()),
             SourceMgr,
...
bool
checkAndConsumeDirectiveWithName(Lexer & Lex, StringRef Name, Token & Tok)
{
...
void
skipComments(Lexer & Lex, Token & Tok)
{
...
// code.
unsigned
getOffsetAfterHeaderGuardsAndComments(StringRef           FileName,
                                      StringRef           Code,
                                      const FormatStyle & Style)
{
...
bool
checkAndConsumeInclusiveDirective(Lexer & Lex, Token & Tok)
{
   auto Matched = [&]() {
      Lex.LexFromRawLexer(Tok);
      return true;
...
unsigned
getMaxHeaderInsertionOffset(StringRef           FileName,
                            StringRef           Code,
                            const FormatStyle & Style)
{
   return getOffsetAfterTokenSequence(
...
bool
isDeletedHeader(llvm::StringRef                   HeaderName,
...
tooling::Replacements
fixCppIncludeInsertions(StringRef                     Code,
                        const tooling::Replacements & Replaces,
                        const FormatStyle &           Style)
{
   ...
   for(auto Line : Lines)
   {
      ...
      if(IncludeRegex.match(Line, &Matches))
      {
         ...
         if(isDeletedHeader(IncludeName, HeadersToDelete))
         {
            ...
            // make sure we don't delete across the file boundary.
...
llvm::Expected<tooling::Replacements>
cleanupAroundReplacements(StringRef                     Code,
                          const tooling::Replacements & Replaces,
                          const FormatStyle &           Style)
{
   // We need to use lambda function here since there are two versions of
   // `cleanup`.
   auto Cleanup = [](const FormatStyle &         Style,
                     StringRef                   Code,
                     std::vector<tooling::Range> Ranges,
                     StringRef FileName) -> tooling::Replacements {
...
tooling::Replacements
reformat(const FormatStyle &       Style,
         StringRef                 Code,
         ArrayRef<tooling::Range>  Ranges,
         StringRef                 FileName,
         FormattingAttemptStatus * Status)
...
tooling::Replacements
cleanup(const FormatStyle &      Style,
        StringRef                Code,
        ArrayRef<tooling::Range> Ranges,
        StringRef                FileName)
{
...
tooling::Replacements
fixNamespaceEndComments(const FormatStyle &      Style,
                        StringRef                Code,
                        ArrayRef<tooling::Range> Ranges,
                        StringRef                FileName)
{
...
tooling::Replacements
sortUsingDeclarations(const FormatStyle &      Style,
                      StringRef                Code,
                      ArrayRef<tooling::Range> Ranges,
                      StringRef                FileName)
{
...
LangOptions
getFormattingLangOpts(const FormatStyle & Style)
{
