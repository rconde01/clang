...
// duplication.
bool
FormatToken::isSimpleTypeSpecifier() const
{
   switch(Tok.getKind())
   {
   case tok::kw_short:
...
void
TokenRole::precomputeFormattingInfos(const FormatToken * Token)
{
}

unsigned
CommaSeparatedList::formatAfterToken(LineState &            State,
                                     ...
                                     bool                   DryRun)
{
   ...
   while(State.NextToken != LBrace->MatchingParen)
   {
      ...
      // If the previous token was one of our commas, we are now on the next
      // item.
...
unsigned
CommaSeparatedList::formatFromToken(LineState &            State,
                                    ContinuationIndenter * Indenter,
                                    bool                   DryRun)
{
   // Formatting with 1 Column isn't really a column layout, so we don't need
   // the special logic here. We can just avoid bin packing any of the
   // parameters.
...
CommaSeparatedList::precomputeFormattingInfos(const FormatToken * Token)
{
   ...
   for(unsigned i = 0, e = Commas.size() + 1; i != e; ++i)
   {
      ...
      {
         ItemEnd = Commas[i];
         // The comma is counted as part of the item when calculating the
         // length.
   ...
   for(unsigned Columns = 1; Columns <= MaxItems; ++Columns)
   {
      ...
      // Don't use this Format, if the difference between the longest and
      // shortest element in a column exceeds a threshold to avoid excessive
      // spaces.
      if([&] {
