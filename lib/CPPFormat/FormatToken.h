...
struct FormatToken
{
   ...
   /// \brief The original 0-based column of this token, including expanded
   /// tabs. The configured TabWidth is used as tab width.
   ...
   /// \brief \c true if this token starts a binary expression, i.e. has at
   /// least one fake l_paren with a precedence greater than prec::Unknown.
   ...
   /// \brief If this token starts a block, this contains all the unwrapped
   /// lines in it.
   ...
   ///
   /// This can be different to Tok.getLocation(), which includes leading
   /// escaped newlines.
...
public:
   ...
   /// \brief After the \c TokenAnnotator has finished annotating all the
   /// tokens, this function precomputes required information for formatting.
