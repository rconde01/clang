...
bool
FormatTokenLexer::tryMergeNSStringLiteral()
{
   if(Tokens.size() < 2)
      return false;
...
bool
FormatTokenLexer::precedesOperand(FormatToken * Tok)
{
   // NB: This is not entirely correct, as an r_paren can introduce an operand
   // location in e.g. `if (foo) /bar/.exec(...);`. That is a rare enough
   // corner case to not matter in practice, though.
   return Tok->isOneOf(tok::period,
                       tok::l_paren,
                       tok::comma,
                       tok::l_brace,
                       tok::r_brace,
...
bool
FormatTokenLexer::canPrecedeRegexLiteral(FormatToken * Prev)
{
   if(!Prev)
      return true;
...
void
FormatTokenLexer::tryParseJSRegexLiteral()
{
...
void
FormatTokenLexer::handleTemplateStrings()
{
   ...
   else if(BacktickToken->is(tok::unknown) && BacktickToken->TokenText == "`")
   {
   ...
   // 'Manually' lex ahead in the current file buffer.
   const char * Offset = Lex->getBufferLocation();
   const char * TmplBegin =
       Offset - BacktickToken->TokenText.size();   // at "`"
