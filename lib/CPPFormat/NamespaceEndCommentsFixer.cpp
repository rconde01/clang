...
static llvm::Regex kNamespaceCommentPattern = llvm::Regex(
...
std::string
computeName(const FormatToken * NamespaceTok)
{
...
std::string
computeEndCommentText(StringRef NamespaceName, bool AddNewline)
{
...
bool
hasEndComment(const FormatToken * RBraceTok)
{
   return RBraceTok->Next && RBraceTok->Next->is(tok::comment);
}

bool
validEndComment(const FormatToken * RBraceTok, StringRef NamespaceName)
{
...
void
addEndComment(const FormatToken *     RBraceTok,
              StringRef               EndCommentText,
              ...
              tooling::Replacements * Fixes)
{
   ...
   auto Err =
       Fixes->add(tooling::Replacement(SourceMgr, Range, EndCommentText));
   if(Err)
   {
...
void
updateEndComment(const FormatToken *     RBraceTok,
                 StringRef               EndCommentText,
                 const SourceManager &   SourceMgr,
                 tooling::Replacements * Fixes)
{
   ...
   auto                Range   = CharSourceRange::getCharRange(
       Comment->getStartOfNonWhitespace(), Comment->Tok.getEndLoc());
   auto Err =
       Fixes->add(tooling::Replacement(SourceMgr, Range, EndCommentText));
   if(Err)
   {
    ...
    FormatTokenLexer &                 Tokens)
{
   ...
   for(size_t I = 0, E = AnnotatedLines.size(); I != E; ++I)
   {
      ...
      if(Style.CompactNamespaces)
      {
            ...
            && !AnnotatedLines[I + 1]->First->Finalized)
         {
            if(hasEndComment(EndCommentPrevTok))
            {
               ...
               updateEndComment(
                   EndCommentPrevTok, std::string(), SourceMgr, &Fixes);
            }
      ...
      // comment token must be. This is either the next token on the current
      // line or the first token on the next line.
