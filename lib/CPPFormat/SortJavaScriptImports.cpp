...
struct JsImportedSymbol
{
   ...
   bool operator==(const JsImportedSymbol & RHS) const
   {
      ...
      // but imports at different code locations are still conceptually the
      // same.
...
bool
operator<(const JsModuleReference & LHS, const JsModuleReference & RHS)
{
           ...
           FormatTokenLexer &                 Tokens) override
   {
      ...
      for(unsigned i = 0, e = Indices.size(); i != e; ++i)
      {
         ...
         if(i + 1 < e)
         {
            ...
            ReferencesText += "\n";
            // Separate imports groups with two line breaks, but keep all
            // exports in a single group.
            if(!Reference.IsExport
               && (Reference.IsExport != References[Indices[i + 1]].IsExport
                   || Reference.Category
                          != References[Indices[i + 1]].Category))
               ReferencesText += "\n";
         }
      }
      ...
      auto Err = Result.add(
          tooling::Replacement(Env.getSourceManager(),
                               CharSourceRange::getCharRange(InsertionPoint),
                               ReferencesText));
      // FIXME: better error handling. For now, just print error message and
      // skip the replacement for the release version.
      if(Err)
      {
...
private:
   ...
   void nextToken()
   {
      ...
      {
         // Set the current token to an invalid token, so that further parsing
         // on this line fails.
   ...
   // Parses module references in the given lines. Returns the module
   // references, and a pointer to the first "main code" line if that is
   // adjacent to the affected lines of module references, nullptr otherwise.
                         ...
                         SmallVectorImpl<AnnotatedLine *> & AnnotatedLines)
   {
      ...
      for(auto Line : AnnotatedLines)
      {
         ...
         if(Start.isInvalid() || References.empty())
            ...
            // part of the import that immediately follows them by using the
            // previously set Start.
                           ...
                           JsModuleReference &        Reference)
   {
      ...
      while(Current->isNot(tok::r_brace))
      {
         ...
         Symbol.Range.setBegin(Current->getPreviousNonComment()
                                   ->Next->WhitespaceRange.getBegin());
         nextToken();
