...
{
public:
   AnnotatingParser(const FormatStyle &        Style,
                    AnnotatedLine &            Line,
...
private:
   bool parseAngle()
   {
      ...
      if(Previous.Previous)
      {
         ...
         if(Previous.Previous->is(tok::r_paren) && Contexts.size() > 1
            && (!Previous.Previous->MatchingPairedToken
                || !Previous.Previous->MatchingPairedToken->is(
      ...
      // a template parameter, not an argument.
      ...
      while(CurrentToken)
      {
         ...
         {
            Left->MatchingPairedToken         = CurrentToken;
            CurrentToken->MatchingPairedToken = Left;
         ...
         if(CurrentToken->isOneOf(tok::r_paren, tok::r_square, tok::r_brace)
            || (CurrentToken->isOneOf(tok::colon, tok::question)
                && InExprContext))
            return false;
         // If a && or || is found and interpreted as a binary operator, this
         // set of angles is likely part of something like "a < b && c > d". If
         // the angles are inside an expression, the ||/&& might also be a
         // binary operator that was misinterpreted because we are parsing
         // template parameters.
   ...
   bool parseParens(bool LookForDecls = false)
   {
                  ...
                  || Left->Previous->is(TT_BinaryOperator)))
      {
         ...
         Contexts.back().IsExpression = true;
              ...
              && Left->Previous->MatchingPairedToken
              && Left->Previous->MatchingPairedToken->is(TT_LambdaLSquare))
      {
      ...
      else if(!Line.MustBeDeclaration && !Line.InPPDirective)
      {
         bool IsForOrCatch =
             Left->Previous
             && Left->Previous->isOneOf(tok::kw_for, tok::kw_catch);
      ...
      while(CurrentToken)
      {
         ...
         if(CurrentToken->is(tok::comma) && CurrentToken->Next
            && !CurrentToken->Next->HasUnescapedNewlineBefore
         ...
         // to parse the type correctly. Reset that after a comma.
         if(CurrentToken->is(tok::comma))
         ...
         if(CurrentToken && CurrentToken->HasUnescapedNewlineBefore)
   ...
   bool parseSquare()
   {
      ...
      bool CppArrayTemplates =
          Parent && Parent->is(TT_TemplateCloser)
      ...
      {
         if(Contexts.back().ContextKind == tok::l_brace && Parent
                 ...
                 && Parent->is(TT_TemplateCloser))
         {
            Left->Type = TT_ArraySubscriptLSquare;
                 ...
                 && Parent->isOneOf(TT_BinaryOperator,
                                    TT_TemplateCloser,
                                    tok::comma,
                                    tok::l_paren,
                                    tok::l_square,
      ...
      ScopedContextCreator ContextCreator(
   ...
   bool consumeToken()
   {
      ...
      case tok::l_paren:
         ...
         if(Tok->Previous && Tok->Previous->is(tok::r_paren)
            && Tok->Previous->MatchingPairedToken
            && Tok->Previous->MatchingPairedToken->is(TT_OverloadedOperatorLParen))
         {
            Tok->Previous->Type                = TT_OverloadedOperator;
            Tok->Previous->MatchingPairedToken->Type = TT_OverloadedOperator;
         ...
         if(Line.MustBeDeclaration && Contexts.size() == 1
            ...
            && (!Tok->Previous
                || !Tok->Previous->isOneOf(tok::kw_decltype,
                                           tok::kw___attribute)))
...
public:
   LineType parseLine()
   {
      ...
      if(Info && Info->getPPKeywordID() == tok::pp_import && CurrentToken->Next
         && CurrentToken->Next->isOneOf(
                tok::string_literal, tok::identifier, tok::kw_static))
      {
         next();
         parseIncludeDirective();
...
private:
   ...
   void resetTokenMetadata(FormatToken * Token)
   {
      ...
      if(!CurrentToken->isOneOf(TT_LambdaLSquare,
                                TT_ForEachMacro,
                                TT_FunctionLBrace,
      ...
      CurrentToken->MatchingPairedToken = nullptr;
   ...
   struct Context
   {
      Context(tok::TokenKind ContextKind,
   ...
   struct ScopedContextCreator
   {
      ...
      ScopedContextCreator(AnnotatingParser & P,
                           tok::TokenKind     ContextKind,
                           unsigned           Increase)
      : P(P)
      {
   ...
   void modifyContext(const FormatToken & Current)
   {
      if(Current.getPrecedence() == prec::Assignment
         && !Line.First->isOneOf(
                tok::kw_template, tok::kw_using, tok::kw_return)
   ...
   void determineTokenType(FormatToken & Current)
   {
      ...
      else if(Current.is(tok::comment))
      {
         if(Current.TokenText.startswith("/*"))
         {
            if(Current.TokenText.endswith("*/"))
               ...
               // The lexer has for some reason determined a comment here. But
               // we cannot really handle it, if it isn't properly terminated.
      ...
      else if(Current.is(tok::r_paren))
      {
         ...
         if(Current.MatchingPairedToken && Current.Next
            ...
            && !Current.Next->isOneOf(tok::semi,
                                      ...
                                      tok::arrow,
                                      tok::coloncolon))
            if(FormatToken * AfterParen = Current.MatchingPairedToken->Next)
            {
               // Make sure this isn't the return type of an Obj-C block
               // declaration
               if(AfterParen->Tok.isNot(tok::caret))
               {
                  if(FormatToken * BeforeParen =
                         Current.MatchingPairedToken->Previous)
                     if(BeforeParen->is(tok::identifier)
                        && BeforeParen->TokenText
                               == BeforeParen->TokenText.upper()
      ...
      else if(Current.is(tok::period))
      {
         ...
         if(PreviousNoComment
            ...
            Current.Type = TT_DesignatedInitializerPeriod;
              ...
              && Current.Previous && !Current.Previous->is(tok::equal)
   ...
   bool isStartOfName(const FormatToken & Tok)
   {
      ...
      if(PreviousNotConst->is(TT_TemplateCloser))
         return PreviousNotConst && PreviousNotConst->MatchingPairedToken
                && PreviousNotConst->MatchingPairedToken->Previous
                && PreviousNotConst->MatchingPairedToken->Previous->isNot(tok::period)
                && PreviousNotConst->MatchingPairedToken->Previous->isNot(
                       tok::kw_template);

      if(PreviousNotConst->is(tok::r_paren) && PreviousNotConst->MatchingPairedToken
         && PreviousNotConst->MatchingPairedToken->Previous
         && PreviousNotConst->MatchingPairedToken->Previous->is(tok::kw_decltype))
         return true;
   ...
   bool rParenEndsCast(const FormatToken & Tok)
   {
      ...
      // line.
      if(Tok.Previous == Tok.MatchingPairedToken || !Tok.Next || !Tok.MatchingPairedToken)
         return false;

      FormatToken * LeftOfParens = Tok.MatchingPairedToken->getPreviousNonComment();
      if(LeftOfParens)
      {
         ...
         {
            if(!LeftOfParens->MatchingPairedToken
               || !LeftOfParens->MatchingPairedToken->Previous)
               return false;
            LeftOfParens = LeftOfParens->MatchingPairedToken->Previous;
         }

         // If there is an identifier (or with a few exceptions a keyword) right
         // before the parentheses, this is unlikely to be a cast.
         if(LeftOfParens->Tok.getIdentifierInfo()
            && !LeftOfParens->isOneOf(Keywords.kw_in,
                                      tok::kw_return,
                                      tok::kw_case,
                                      tok::kw_delete))
            return false;
         ...
         // that this cannot be a cast.
         if(LeftOfParens->isOneOf(tok::r_square,
                                  TT_OverloadedOperator,
                                  TT_TemplateCloser,
                                  tok::ellipsis))
            return false;
      ...
      // cast.
      for(const FormatToken * Token = Tok.MatchingPairedToken->Next; Token != &Tok;
          Token                     = Token->Next)
      ...
      for(FormatToken * Prev = Tok.Previous; Prev != Tok.MatchingPairedToken;
          Prev               = Prev->Previous)
      {
                                   ...
                                   bool                InTemplateArgument)
   {
      ...
      if(!NextToken || NextToken->isOneOf(tok::arrow, tok::equal, tok::kw_const)
      ...
      if(PrevToken->isOneOf(tok::l_paren,
                            tok::l_square,
                            tok::l_brace,
                            tok::comma,
                            tok::semi,
                            ...
                            tok::kw_delete,
                            tok::kw_sizeof)
         || PrevToken->isOneOf(TT_BinaryOperator,
                               TT_ConditionalExpr,
                               TT_UnaryOperator,
                               TT_CastRParen))
         return TT_UnaryOperator;
      ...
      if(PrevToken->is(tok::r_paren) && PrevToken->MatchingPairedToken
         && PrevToken->MatchingPairedToken->Previous
         && PrevToken->MatchingPairedToken->Previous->isOneOf(tok::kw_typeof,
                                                        tok::kw_decltype))
         return TT_PointerOrReference;

      if(PrevToken->Tok.isLiteral()
         || PrevToken->isOneOf(tok::r_paren,
                               tok::r_square,
                               tok::kw_true,
                               tok::kw_false,
                               tok::r_brace)
   ...
   TokenType determinePlusMinusCaretUsage(const FormatToken & Tok)
   {
      ...
      if(PrevToken->isOneOf(tok::equal,
                            tok::l_paren,
                            tok::comma,
                            tok::l_square,
                            tok::question,
   ...
   // parseAngle determines that a specific token can't be a template opener, it
   // will make same decision irrespective of the decisions for tokens leading
   // up to it. Store this information to prevent this from causing exponential
   // runtime.
...
{
public:
   ExpressionParser(const FormatStyle &        Style,
                    const AdditionalKeywords & Keywords,
                    AnnotatedLine &            Line)
   ...
   void parse(int Precedence = 0)
   {
      ...
      while(Current)
      {
         ...
         if(!Current || (Current->closesScope() && Current->MatchingPairedToken)
...
private:
   ...
   void next(bool SkipPastLeadingComments = true)
   {
      ...
      while(Current && (Current->UserNewlinesBefore == 0 || SkipPastLeadingComments)
...
TokenAnnotator::setCommentLineLevels(SmallVectorImpl<AnnotatedLine *> & Lines)
{
       ...
       ++I)
   {
      ...
      if(NextNonCommentLine && CommentLine)
      {
         ...
         // following it, that's probably intentional and we should keep it.
         bool AlignedWithNextLine =
             NextNonCommentLine->First->UserNewlinesBefore <= 1
...
static unsigned
maxNestingDepth(const AnnotatedLine & Line)
{
   unsigned Result = 0;
...
void
TokenAnnotator::annotate(AnnotatedLine & Line)
{
...
static bool
isFunctionDeclarationName(const FormatToken &   Current,
                          const AnnotatedLine & Line)
{
   ...
   if(!Next || !Next->is(tok::l_paren) || !Next->MatchingPairedToken)
      return false;
   // If the lines ends with "{", this is likely an function definition.
   if(Line.Last->is(tok::l_brace))
      return true;
   if(Next->Next == Next->MatchingPairedToken)
      return true;   // Empty parentheses.
   // If there is an &/&& after the r_paren, this is likely a function.
   if(Next->MatchingPairedToken->Next
      && Next->MatchingPairedToken->Next->is(TT_PointerOrReference))
      return true;
   for(const FormatToken * Tok = Next->Next; Tok && Tok != Next->MatchingPairedToken;
       Tok                     = Tok->Next)
   {
      if(Tok->is(tok::l_paren) && Tok->MatchingPairedToken)
      {
         Tok = Tok->MatchingPairedToken;
         continue;
      }
...
bool
TokenAnnotator::mustBreakForReturnType(const AnnotatedLine & Line) const
{
...
TokenAnnotator::calculateFormattingInformation(AnnotatedLine & Line)
{
   ...
   Line.First->TotalLength =
       Line.First->IsMultiline ? Style.ColumnLimit : Line.First->FirstLineColumnWidth;
   ...
   while(Current)
   {
      ...
      if(Current->is(TT_LineComment))
      {
         ...
         // If we find a trailing comment, iterate backwards to determine
         // whether it seems to relate to a specific parameter. If so, break
         // before that parameter to avoid changing the comment's meaning. E.g.
         // don't move 'b' to the previous line in:
         //   SomeFunction(a,
         //                b, // comment
         //                c);
         if(!Current->HasUnescapedNewlineBefore)
         {
                ...
                Parameter               = Parameter->Previous)
            {
               ...
               if(Parameter->Previous && Parameter->Previous->is(tok::comma))
               {
                  if(!Parameter->Previous->is(TT_CtorInitializerComma)
                     && Parameter->HasUnescapedNewlineBefore)
      ...
      if(Current->MustBreakBefore || Prev->Children.size() > 1
         ...
         Current->TotalLength = Prev->TotalLength + Current->FirstLineColumnWidth
   ...
   for(Current = Line.First; Current != nullptr; Current = Current->Next)
   {
      ...
      if(Current->MatchingPairedToken
         && Current->MatchingPairedToken->opensBlockOrBlockTypeList(Style))
      {
   ...
   while(Current)
   {
      ...
      else
      {
         UnbreakableTailLength +=
             Current->FirstLineColumnWidth + Current->SpacesRequiredBefore;
      }
...
unsigned
TokenAnnotator::splitPenalty(const AnnotatedLine & Line,
                             ...
                             bool                  InFunctionDecl)
{
   ...
   if(Left.isOneOf(TT_RangeBasedForLoopColon,
                   TT_InheritanceColon,
   ...
   {
      // Breaking before the "./->" of a chained call/member access is
      // reasonably cheap, as formatting those with one call per line is
      // generally desirable. In particular, it should be cheaper to break
      // before the call than it is to break inside a call's parameters, which
      // could lead to weird "hanging" indents. The exception is the very last
      // "./->" to support this frequent pattern:
      //
      ...
      // call.
      //
      // Also apply higher penalty is not a call as that might lead to a
      // wrapping like:
      //
      //   aaaaaaa
   ...
   if(Right.is(tok::lessless))
   {
      ...
      if(!Left.is(tok::r_paren) || Right.OperatorIndex > 0)
         // Slightly prefer to break before the first one in log-like
         // statements.
         return 2;
      return 1;
   }
                                     ...
                                     const FormatToken &   Right)
{
   ...
   if(Left.is(tok::l_paren) || Right.is(tok::r_paren))
      return (Right.is(TT_CastRParen)
              || (Left.MatchingPairedToken && Left.MatchingPairedToken->is(TT_CastRParen)))
   ...
   if(Right.is(tok::r_square))
      return Right.MatchingPairedToken
             && ((Style.SpacesInContainerLiterals
                  && Right.MatchingPairedToken->is(TT_ArrayInitializerLSquare))
                 || (Style.SpacesInSquareBrackets
                     && Right.MatchingPairedToken->is(TT_ArraySubscriptLSquare)));
   if(Right.is(tok::l_square)
      && !Right.isOneOf(TT_LambdaLSquare, TT_DesignatedInitializerLSquare)
      && !Left.is(tok::numeric_constant))
      return false;
   if(Left.is(tok::l_brace) && Right.is(tok::r_brace))
      return !Left.Children.empty();   // No spaces in "{}".
   if((Left.is(tok::l_brace) && Left.BlockKind != BK_Block)
      || (Right.is(tok::r_brace) && Right.MatchingPairedToken
          && Right.MatchingPairedToken->BlockKind != BK_Block))
   ...
   if(Right.is(TT_UnaryOperator))
      return !Left.isOneOf(tok::l_paren, tok::l_square);
   if((Left.isOneOf(tok::identifier, tok::greater, tok::r_square, tok::r_paren)
   ...
   if(Right.is(tok::hash) && Left.is(tok::identifier) && Left.TokenText == "L")
      return false;
   if(Left.is(TT_TemplateCloser) && Left.MatchingPairedToken
      && Left.MatchingPairedToken->Previous
      && Left.MatchingPairedToken->Previous->is(tok::period))
      // A.<B>DoSomething();
      return false;
...
bool
TokenAnnotator::spaceRequiredBefore(const AnnotatedLine & Line,
                                    const FormatToken &   Right)
{
   ...
   if(Left.is(TT_ImplicitStringLiteral))
      return Right.PrecedingWhitespaceRange.getBegin() != Right.PrecedingWhitespaceRange.getEnd();
   ...
   if(Right.is(tok::colon))
   {
      if(Line.First->isOneOf(tok::kw_case, tok::kw_default)
         ...
         || Right.getNextNonComment()->is(tok::semi))
         return false;
   ...
   if(Left.is(TT_TemplateCloser) && Right.is(tok::l_paren)
      ...
      return Style.SpaceBeforeParens == FormatStyle::SBPO_Always;
   if(Right.is(TT_TemplateOpener) && Left.is(tok::r_paren) && Left.MatchingPairedToken
      && Left.MatchingPairedToken->is(TT_OverloadedOperatorLParen))
      return false;
...
static bool
isAllmanBrace(const FormatToken & Tok)
{
   return Tok.is(tok::l_brace) && Tok.BlockKind == BK_Block;
}

bool
TokenAnnotator::mustBreakBefore(const AnnotatedLine & Line,
                                const FormatToken &   Right)
{
   const FormatToken & Left = *Right.Previous;
   if(Right.UserNewlinesBefore > 1 && Style.MaxEmptyLinesToKeep > 0)
      return true;
   ...
   if(Line.Type != LT_ImportStatement)
   {
      const FormatToken * BeforeClosingBrace = nullptr;
      if(Left.isOneOf(tok::l_brace, TT_ArrayInitializerLSquare)
         && Left.BlockKind != BK_Block && Left.MatchingPairedToken)
         BeforeClosingBrace = Left.MatchingPairedToken->Previous;
      else if(Right.MatchingPairedToken
              && Right.MatchingPairedToken->isOneOf(tok::l_brace,
                                              TT_ArrayInitializerLSquare))
         BeforeClosingBrace = &Left;
      if(BeforeClosingBrace
         && (BeforeClosingBrace->is(tok::comma)
   ...
   if(Right.is(tok::comment))
      return Left.BlockKind != BK_BracedInit
             ...
             && (Right.UserNewlinesBefore > 0 && Right.HasUnescapedNewlineBefore);
   ...
   if(Right.is(tok::lessless) && Right.Next
      ...
      return true;
   if(Right.Previous->ClosesTemplateDeclaration && Right.Previous->MatchingPairedToken
      && Right.Previous->MatchingPairedToken->NestingLevel == 0
   ...
   if(Right.is(tok::string_literal) && Right.TokenText.startswith("R\""))
      ...
      return Right.UserNewlinesBefore > 0;

   if(Right.is(TT_InlineASMBrace))
      return Right.HasUnescapedNewlineBefore;
...
bool
TokenAnnotator::canBreakBefore(const AnnotatedLine & Line,
                               const FormatToken &   Right)
{
   const FormatToken & Left = *Right.Previous;
   ...
   if(Right.is(tok::r_paren) || Right.is(TT_TemplateCloser))
      return false;
   if(Right.is(tok::r_square) && Right.MatchingPairedToken
      && Right.MatchingPairedToken->is(TT_LambdaLSquare))
      return false;

   // We only break before r_brace if there was a corresponding break before
   // the l_brace, which is tracked by BreakBeforeClosingBrace.
   if(Right.is(tok::r_brace))
      return Right.MatchingPairedToken && Right.MatchingPairedToken->BlockKind == BK_Block;

   // Allow breaking after a trailing annotation, e.g. after a method
   // declaration.
   if(Left.is(TT_TrailingAnnotation))
      return !Right.isOneOf(tok::l_brace,
                            tok::semi,
                            tok::equal,
                            tok::l_paren,
                            tok::less,
   ...
   return Left.isOneOf(tok::comma,
          ...
          || Right.isOneOf(TT_TrailingReturnArrow,
                           TT_LambdaArrow,
                           tok::lessless,
                           tok::colon,
                           tok::l_square)
...
void
TokenAnnotator::printDebugInfo(const AnnotatedLine & Line)
{
