...
bool
startsExternCBlock(const AnnotatedLine & Line)
{
...
public:
   ...
   /// \brief Update the indent state given that \p Line is going to be
   /// formatted next.
...
bool
isNamespaceDeclaration(const AnnotatedLine * Line)
{
...
bool
isEndOfNamespace(const AnnotatedLine *                    Line,
...
{
public:
   LineJoiner(const FormatStyle &                      Style,
              const AdditionalKeywords &               Keywords,
   ...
   /// \brief Returns the next line, merging multiple lines into one if
   /// possible.
...
private:
       ...
       unsigned                                         Limit)
   {
      ...
      if(1 + I[1]->Last->TotalLength > Limit)
         return 0;
      if(I[1]->First->isOneOf(
             tok::semi, tok::kw_if, tok::kw_for, tok::kw_while, TT_LineComment))
         return 0;
                           ...
                           unsigned Limit)
   {
      ...
      for(; NumStmts < 3; ++NumStmts)
      {
         ...
         if(Line->First->isOneOf(tok::kw_case, tok::kw_default, tok::r_brace))
            break;
         if(Line->First->isOneOf(tok::kw_if,
                                 tok::kw_for,
                                 tok::kw_switch,
                                 tok::kw_while,
                                 tok::comment)
                       ...
                       unsigned                                         Limit)
   {
                             ...
                             Keywords.kw___except))
      {
         ...
         if(Line.First->isOneOf(tok::kw_try,
                                tok::kw___try,
                                tok::kw_catch,
                                Keywords.kw___except,
                                tok::kw___finally))
...
#ifndef NDEBUG
static void
printLineState(const LineState & State)
{
   llvm::dbgs() << "State: ";
...
{
public:
   LineFormatter(ContinuationIndenter *   Indenter,
...
protected:
   ...
   /// \c false), the entire block has to be kept on the same line (which is
   ...
   /// put the closing "}" on yet another new line.
   ///
                       ...
                       unsigned &  Penalty)
   {
      ...
      if(NewLine)
      {
         ...
         Penalty += BlockFormatter->format(Previous.Children,
                                           DryRun,
                                           AdditionalIndent,
      ...
      if(!DryRun)
      {
         Whitespaces->replaceWhitespace(*Child->First,
                                        /*Newlines=*/0,
                                        /*Spaces=*/1,
...
public:
   ...
   /// decisions.
   unsigned formatLine(const AnnotatedLine & Line,
                       unsigned              FirstIndent,
...
private:
   ...
   unsigned analyzeSolutionSpace(LineState & InitialState, bool DryRun)
   {
      ...
      while(!Queue.empty())
      {
         ...
         if(LastFormat == FD_Unformatted || LastFormat == FD_Continue)
            addNextStateToQueue(
                Penalty, Node, /*NewLine=*/false, &Count, &Queue);
         if(LastFormat == FD_Unformatted || LastFormat == FD_Break)
            addNextStateToQueue(
                Penalty, Node, /*NewLine=*/true, &Count, &Queue);
      }
   ...
   void addNextStateToQueue(unsigned    Penalty,
                               ...
                               bool FixBadIndentation)
{
   ...
   unsigned           Penalty = 0;
   LevelIndentTracker IndentTracker(
       Style, Keywords, Lines[0]->Level, AdditionalIndent);
       ...
       Line = NextLine)
   {
      ...
      // we exit the scope of affected lines to prevent indenting a the entire
      // remaining file if it currently missing a closing brace.
      bool ContinueFormatting = TheLine.Level > RangeMinLevel
      ...
      if(ShouldFormat && TheLine.Type != LT_Invalid)
      {
         ...
         if(Style.ColumnLimit == 0)
            ...
            Penalty +=
                OptimizingLineFormatter(Indenter, Whitespaces, Style, this)
                    .formatLine(TheLine, Indent, DryRun);
      ...
      else
      {
         ...
         if(!DryRun)
         {
            ...
            if(ReformatLeadingWhitespace)
               formatFirstToken(
                   TheLine, PreviousLine, TheLine.First->OriginalColumn);
            else
...
void
UnwrappedLineFormatter::formatFirstToken(const AnnotatedLine & Line,
