...
{
public:
   ScopedDeclarationState(UnwrappedLine &     Line,
...
static bool
isLineComment(const FormatToken & FormatTok)
{
...
static bool
continuesLineComment(const FormatToken & FormatTok,
                     const FormatToken * Previous,
                     const FormatToken * MinColumnToken)
{
   if(!Previous || !MinColumnToken)
      return false;
   unsigned MinContinueColumn = MinColumnToken->OriginalColumn
...
{
public:
   ScopedMacroState(UnwrappedLine &      Line,
                    FormatTokenSource *& TokenSource,
                    FormatToken *&       ResetToken)
   : Line(Line),
     TokenSource(TokenSource),
     ResetToken(ResetToken),
     PreviousLineLevel(Line.Level),
...
UnwrappedLineParser::parseFile()
{
   ...
   ScopedDeclarationState DeclarationState(
       *Line, DeclarationScopeStack, MustBeDeclaration);
...
void
UnwrappedLineParser::parseLevel(bool HasOpeningBrace)
{
   ...
   do
   {
      ...
      case tok::l_brace:
         ...
         // must be in a non-declaration context.
...
void
UnwrappedLineParser::calculateBraceTypes(bool ExpectClassBody)
{
      ...
      case tok::l_brace:
         if(Style.Language == FormatStyle::LK_JavaScript && PrevTok)
         {
            if(PrevTok->is(tok::colon))
               ...
               // BK_Block. In both cases, this must be parsed as an inline
               // braced init.
               Tok->BlockKind = BK_BracedInit;
            else if(PrevTok->is(tok::r_paren))
               // `) { }` can only occur in function or method declarations in
               // JS.
      ...
      case tok::r_brace:
         ...
         if(LBraceStack.back()->BlockKind == BK_Unknown)
         {
            ...
            else
            {
               ...
               // If there is a comma, semicolon or right paren after the
               // closing brace, we assume this is a braced initializer list.
               // Note that regardless how we mark inner braces here, we will
               // overwrite the BlockKind later if we parse a braced list (where
               // all blocks inside are by default braced lists), or when we
               // explicitly detect blocks (for example while parsing lambdas).
               // FIXME: Some of these do not apply to JS, e.g. "} {" can never
               // be a braced list in JS.
               ProbablyBracedList =
                   (Style.Language == FormatStyle::LK_JavaScript
                    && NextTok->isOneOf(
                           Keywords.kw_of, Keywords.kw_in, Keywords.kw_as))
                   || (Style.isCpp() && NextTok->is(tok::l_paren))
                   || NextTok->isOneOf(tok::comma,
                                       tok::period,
                                       tok::colon,
                                       tok::r_paren,
                                       tok::r_square,
                   ...
                   || (NextTok->is(tok::identifier)
                       && !PrevTok->isOneOf(
                              tok::semi, tok::r_brace, tok::l_brace))
                   || (NextTok->is(tok::semi)
...
static bool
isGoogScope(const UnwrappedLine & Line)
{
...
static bool
isIIFE(const UnwrappedLine & Line, const AdditionalKeywords & Keywords)
{
...
static bool
ShouldBreakBeforeBrace(const FormatStyle & Style,
...
void
UnwrappedLineParser::parseChildBlock()
{
   FormatTok->BlockKind = BK_Block;
   nextToken();
   {
...
void
UnwrappedLineParser::parsePPDirective()
{
...
void
UnwrappedLineParser::conditionalCompilationEnd()
{
   assert(PPBranchLevel < (int)PPLevelBranchIndex.size());
...
void
UnwrappedLineParser::parsePPIf(bool IfDef)
{
...
void
UnwrappedLineParser::parsePPElse()
{
...
void
UnwrappedLineParser::parsePPElIf()
{
   parsePPElse();
}

void
UnwrappedLineParser::parsePPEndIf()
{
...
void
UnwrappedLineParser::parsePPDefine()
{
   nextToken();
...
static bool
tokenCanStartNewLine(const clang::Token & Tok)
{
...
static bool
mustBeJSIdent(const AdditionalKeywords & Keywords,
              const FormatToken *        FormatTok)
{
   // FIXME: This returns true for C/C++ keywords like 'struct'.
   return FormatTok->is(tok::identifier)
          && (FormatTok->Tok.getIdentifierInfo() == nullptr
              || !FormatTok->isOneOf(Keywords.kw_in,
                                     Keywords.kw_of,
                                     Keywords.kw_as,
                                     Keywords.kw_async,
                                     Keywords.kw_await,
...
static bool
mustBeJSIdentOrValue(const AdditionalKeywords & Keywords,
                     const FormatToken *        FormatTok)
{
...
static bool
isJSDeclOrStmt(const AdditionalKeywords & Keywords,
               const FormatToken *        FormatTok)
{
   return FormatTok->isOneOf(tok::kw_return,
                             Keywords.kw_yield,
                             // conditionals
                             tok::kw_if,
                             tok::kw_else,
...
void
UnwrappedLineParser::readTokenWithJavaScriptASI()
{
   ...
   {
      // If the token before the previous one is an '@', the previous token is
      // an annotation and can precede another identifier/value.
   ...
   if(NextMustBeValue && !NextEndsTemplateExpr && !PreviousStartsTemplateExpr
      && (PreviousMustBeValue
          || Previous->isOneOf(
                 tok::r_square, tok::r_paren, tok::plusplus, tok::minusminus)))
      return addUnwrappedLine();
...
UnwrappedLineParser::parseStructuralElement()
{
   ...
   case tok::identifier:
      ...
      if(Style.isCpp()
         && FormatTok->isOneOf(Keywords.kw_signals,
                               Keywords.kw_qsignals,
                               Keywords.kw_slots,
                               Keywords.kw_qslots))
      {
         nextToken();
   ...
   do
   {
      ...
      case tok::kw_typedef:
         nextToken();
         if(FormatTok->isOneOf(Keywords.kw_NS_ENUM,
                               Keywords.kw_NS_OPTIONS,
                               Keywords.kw_CF_ENUM,
      ...
      case tok::kw_union:
      case tok::kw_class:
         // parseRecord falls through and does not yet add an unwrapped line as
         // a record declaration or definition can start a structural element.
         parseRecord();
      ...
      case tok::period:
         ...
         if(Style.Language == FormatStyle::LK_JavaScript && FormatTok
            ...
            // appear in "IdentifierName" positions. See
      ...
      case tok::l_brace:
         if(!tryToParseBracedList())
         {
            ...
            // projections for them (the one we know is missing are lambdas).
      ...
      case tok::identifier:
      {
         ...
         // Function declarations (as opposed to function expressions) are
         // parsed on their own unwrapped line by continuing this loop. Function
         // expressions (functions that are not on their own line) must not
         // create a new unwrapped line, so they are special cased below.
         size_t TokenCount = Line->Tokens.size();
         if(Style.Language == FormatStyle::LK_JavaScript
            && FormatTok->is(Keywords.kw_function)
            && (TokenCount > 1
                || (TokenCount == 1
                    && !Line->Tokens.front().Tok->is(Keywords.kw_async))))
         {
            ...
            && FormatTok->is(Keywords.kw_interface))
         {
            if(Style.Language == FormatStyle::LK_JavaScript)
            {
               // In JavaScript/TypeScript, "interface" can be used as a
               // standalone identifier, e.g. in `var interface = 1;`. If
               // "interface" is followed by another identifier, it is very like
               // to be an actual interface declaration.
               unsigned      StoredPosition = Tokens->getPosition();
         ...
         if(Line->Tokens.size() == 1 &&
            // JS doesn't have macros, and within classes colons indicate
            // fields, not labels.
            Style.Language != FormatStyle::LK_JavaScript)
         {
            ...
            // as well as free-standing macros like Q_OBJECT.
...
UnwrappedLineParser::tryToParseLambda()
{
   ...
   if(Previous
      && (Previous->isOneOf(
              tok::identifier, tok::kw_operator, tok::kw_new, tok::kw_delete)
...
bool
UnwrappedLineParser::tryToParseLambdaIntroducer()
{
   nextToken();
...
void
UnwrappedLineParser::tryToParseJSFunction()
{
   assert(
       FormatTok->is(Keywords.kw_function)
...
bool
UnwrappedLineParser::tryToParseBracedList()
{
...
bool
UnwrappedLineParser::parseBracedList(bool           ContinueOnSemicolons,
                                     tok::TokenKind ClosingBraceKind)
{
   ...
   do
   {
      if(Style.Language == FormatStyle::LK_JavaScript)
      {
         ...
         {
            nextToken();
            // Fat arrows can be followed by simple expressions or by child
      ...
      case tok::l_paren:
         parseParens();
         // JavaScript can just have free standing methods and getters/setters
         // in object literals. Detect them by a "{" following ")".
         if(Style.Language == FormatStyle::LK_JavaScript)
         {
            if(FormatTok->is(tok::l_brace))
               parseChildBlock();
...
void
UnwrappedLineParser::parseSquare()
{
...
void
UnwrappedLineParser::parseIfThenElse()
{
...
void
UnwrappedLineParser::parseTryCatch()
{
   ...
   while(1)
   {
      ...
      if(!(FormatTok->isOneOf(
               tok::kw_catch, Keywords.kw___except, tok::kw___finally)
           || ((Style.Language == FormatStyle::LK_Java
                || Style.Language == FormatStyle::LK_JavaScript)
...
void
UnwrappedLineParser::parseNamespace()
{
   ...
   if(FormatTok->Tok.is(tok::l_brace))
   {
      ...
      // would think. Puttin the semicolon into its own line is very ugly.
...
void
UnwrappedLineParser::parseForOrWhileLoop()
{
...
void
UnwrappedLineParser::parseDoWhile()
{
...
void
UnwrappedLineParser::parseLabel()
{
   nextToken();
...
void
UnwrappedLineParser::parseCaseLabel()
{
...
void
UnwrappedLineParser::parseSwitch()
{
...
void
UnwrappedLineParser::parseAccessSpecifier()
{
   nextToken();
...
void
UnwrappedLineParser::parseJavaEnumBody()
{
...
void
UnwrappedLineParser::parseRecord(bool ParseAsExpr)
{
   const FormatToken & InitialToken = *FormatTok;
   nextToken();
   ...
   while(FormatTok->isOneOf(tok::identifier,
                            tok::coloncolon,
                            tok::hashhash,
                            tok::kw___attribute,
                            tok::kw___declspec,
...
void
UnwrappedLineParser::parseObjCProtocolList()
{
...
void
UnwrappedLineParser::parseObjCUntilAtEnd()
{
   do
   {
...
void
UnwrappedLineParser::parseObjCProtocol()
{
   nextToken();
...
printDebugInfo(const UnwrappedLine & Line, StringRef Prefix = "")
{
...
void
UnwrappedLineParser::addUnwrappedLine()
{
...
bool
UnwrappedLineParser::eof() const
{
   return FormatTok->Tok.is(tok::eof);
}

bool
UnwrappedLineParser::isOnNewLine(const FormatToken & FormatTok)
{
                            ...
                            llvm::Regex &         CommentPragmasRegex)
{
   ...
   return continuesLineComment(
       FormatTok, /*Previous=*/Line.Tokens.back().Tok, MinColumnToken);
}

void
UnwrappedLineParser::flushComments(bool NewlineBeforeNext)
{
...
const FormatToken *
UnwrappedLineParser::getPreviousToken()
{
...
void
UnwrappedLineParser::distributeComments(
    ...
    const FormatToken *                    NextTok)
{
   ...
   // such that each comment line from the trail is aligned with the next token,
   // if the next token exists. If a trail exists, the beginning of the maximal
...
void
UnwrappedLineParser::pushToken(FormatToken * Tok)
{
