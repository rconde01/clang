...
: Tok(&Tok),
  ...
  TokenLength(0),
  PreviousEndOfTokenColumn(0),
  EscapedNewlineColumn(0),
  StartOfBlockComment(nullptr),
  IndentationOffset(0)
{
}

void
WhitespaceManager::replaceWhitespace(FormatToken & Tok,
                                     unsigned      Newlines,
                                     unsigned      Spaces,
                                     unsigned      StartOfTokenColumn,
                                     bool          InPPDirective)
...
void
WhitespaceManager::addUntouchableToken(const FormatToken & Tok,
                                       bool                InPPDirective)
{
   if(Tok.Finalized)
      return;
...
void
WhitespaceManager::replaceWhitespaceInToken(const FormatToken & Tok,
                                            unsigned            Offset,
                                            ...
                                            int                 Spaces)
{
   ...
   Changes.push_back(
       Change(Tok,
              /*CreateReplacement=*/true,
              SourceRange(Start, Start.getLocWithOffset(ReplaceChars)),
              Spaces,
              std::max(0, Spaces),
              Newlines,
              PreviousPostfix,
...
WhitespaceManager::calculateLineBreakInformation()
{
   ...
   for(unsigned i = 1, e = Changes.size(); i != e; ++i)
   {
      ...
      // #endif // line 3
      //
      // one of the runs will produce the sequence of lines marked with line 1,
      // 2 and 3. So the two consecutive whitespace changes just before '// line
      // 2' and before '#endif // line 3' span multiple lines and tokens:
      //
      ...
      Changes[i - 1].IsTrailingComment =
          ...
          // the aligning of trailing comments. Consider the case where we
          // reflow the second line up in this example:
          //
          // // line 1
          // // line 2
          //
          // That amounts to 2 changes by BreakableLineCommentSection:
          //  - the first, delimited by (), for the whitespace between the
          //  tokens, - and second, delimited by [], for the whitespace at the
          ...
          // In this case, the below check ensures that the second change
          // doesn't get treated as a trailing comment change here, since this
          // might trigger additional whitespace to be wrongly inserted before
          // "line 2" by the comment aligner here.
          //
          // For a proper solution we need a mechanism to say to
          // WhitespaceManager that a particular change breaks the current
          // sequence of trailing comments.
   ...
   {
      // Reset the IsTrailingComment flag for changes inside of trailing
      // comments so they don't get realigned later. Comment line breaks however
...
template<typename F>
static void
AlignTokenSequence(unsigned                                     Start,
                   unsigned                                     End,
                   unsigned                                     Column,
                   F &&                                         Matches,
...
template<typename F>
static unsigned
AlignTokens(const FormatStyle &                          Style,
            F &&                                         Matches,
            SmallVector<WhitespaceManager::Change, 16> & Changes,
            unsigned                                     StartAt)
{
   ...
   // only align a sequence of matching tokens if they are preceded by the same
   // number of commas.
   ...
   auto AlignCurrentSequence = [&] {
      if(StartOfSequence > 0 && StartOfSequence < EndOfSequence)
         AlignTokenSequence(
             StartOfSequence, EndOfSequence, MinColumn, Matches, Changes);
      MinColumn       = 0;
      MaxColumn       = UINT_MAX;
      StartOfSequence = 0;
      EndOfSequence   = 0;
...
WhitespaceManager::alignConsecutiveDeclarations()
{
   ...
   // done about it, but it raises the question of alignment of code like:
   //   const char* const* v1;
   //   float const* v2;
   //   SomeVeryLongType const& v3;
   AlignTokens(Style,
               [](Change const & C) {
                  // tok::kw_operator is necessary for aligning operator
...
void
WhitespaceManager::alignTrailingComments(unsigned Start,
                                         unsigned End,
                                         unsigned Column)
{
   for(unsigned i = Start; i != End; ++i)
...
void
WhitespaceManager::alignEscapedNewlines(unsigned Start,
                                        unsigned End,
                                        unsigned Column)
{
   for(unsigned i = Start; i < End; ++i)
...
WhitespaceManager::generateChanges()
{
   for(unsigned i = 0, e = Changes.size(); i != e; ++i)
   {
      ...
      if(C.CreateReplacement)
      {
         ...
         appendIndentText(ReplacementText,
                          C.Tok->IndentLevel,
                          std::max(0, C.Spaces),
...
void
WhitespaceManager::storeReplacement(SourceRange Range, StringRef Text)
{
   unsigned WhitespaceLength = SourceMgr.getFileOffset(Range.getEnd())
                               - SourceMgr.getFileOffset(Range.getBegin());
   // Don't create a replacement, if it does not change anything.
   if(StringRef(SourceMgr.getCharacterData(Range.getBegin()), WhitespaceLength)
      == Text)
      return;
...
void
WhitespaceManager::appendNewlineText(std::string & Text, unsigned Newlines)
{
...
void
WhitespaceManager::appendNewlineText(std::string & Text,
                                     unsigned      Newlines,
...
void
WhitespaceManager::appendIndentText(std::string & Text,
                                    ...
                                    unsigned      WhitespaceStartColumn)
{
   ...
   case FormatStyle::UT_ForIndentation:
      if(WhitespaceStartColumn == 0)
      {
         ...
         // This happens, e.g. when a line in a block comment is indented less
         // than the first one.
