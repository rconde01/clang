...
{
public:
   WhitespaceManager(const SourceManager & SourceMgr,
   ...
   /// alignment here. In all other cases negative \p Spaces will be truncated
   /// to 0.
   ///
   /// When \p InPPDirective is true, escaped newlines are inserted. \p Spaces
   /// is used to align backslashes correctly.
   void replaceWhitespaceInToken(const FormatToken & Tok,
                                 unsigned            Offset,
   ...
   struct Change
   {
      ...
      /// \p PreviousLinePostfix, \p NewlinesBefore line breaks, \p Spaces
      /// spaces and \p CurrentLinePrefix.
      ///
      /// \p StartOfTokenColumn and \p InPPDirective will be used to lay out
      /// trailing comments and escaped newlines.
      Change(const FormatToken & Tok,
      ...
      // The kind of the token whose whitespace this change replaces, or in
      // which this change inserts whitespace.
      // FIXME: Currently this is not set correctly for breaks inside comments,
      // as the \c BreakableToken is still doing its own alignment.
      ...
      std::pair<unsigned, unsigned> nestingAndIndentLevel() const
      {
...
private:
   ...
   void appendNewlineText(std::string & Text,
                          ...
                          unsigned      PreviousEndOfTokenColumn,
                          unsigned      EscapedNewlineColumn);
   void appendIndentText(std::string & Text,
